...
;;; エントリーポイント
(define (main args)
    (print (matrix-ref grid 1 1))
    (print (seek-vertical grid 1 1))
;   (print (euler11))
  0)
;;; 実装
(define (euler11)
...
(define (matrix-ref grid column row)
  (cond
    ...
    ((or (> column 20) (> row 20))
     1)
    (else (list-ref-sp grid (+ (* column 20) row)))))

(define (seek-horizon grid x y)
  (list
    (map (lambda (y) (matrix-ref grid x y)) #?=(iota 4 x -1))
    (map (lambda (y) (matrix-ref grid x y)) #?=(iota 4 x 1))
    ))
(define (__seek-vertical grid x y)
  (list
    (map (lambda (x) (matrix-ref grid x y)) #?=(iota 4 x -1))
    (map (lambda (x) (matrix-ref grid x y)) #?=(iota 4 x 1))
    ))
(define (seek-vertical-SE grid x y)
  (list
    (map (lambda (x) (matrix-ref grid x x)) #?=(iota 4 x -1))
    (map (lambda (x) (matrix-ref grid x x)) #?=(iota 4 x 1))
    ))
