...
- 累計日数は （+ 開始日からその年の正月までの累積日数 正月からその日までの累積日数)
...
(define (main args)
  (cond
    ;; テスト
    ((not (eqv? (prog-days 1900 1 1) 1))
      (error "prog-days Error!" (prog-days 1900 1 1)))
    ((not (eqv? (prog-days 1900 3 1) 60))
      (error "prog-days Error!" (prog-days 1900 3 1)))
    ((not (equal? (day->planet (prog-days 1900 1 1)) "Mo."))
      (error "day->planet Error! 1 " (day->planet (prog-days 1900 1 1))))
    ((not (equal? (day->planet (prog-days 1901 1 1)) "Mo."))
      (error "day->planet Error! 2"))
    ((not (equal? (take (reverse generate-dates) 3) '((2000 12 1) (2000 11 1) (2000 10 1))))
      (error "generate-dates Error!"))
...
(define (prog-years year)
  (-
    (apply + (map (lambda (year) (if (leap? year) 366 365)) twenty-century-and-one))
    (apply + (map (lambda (year) (if (leap? year) 366 365)) (remaining-years year)))))

; うるう年を考慮した正月からこの月この日までの総日数
...
(define (day->planet n)
  (let ((days-of-week (mod n 7)))
    (cond
      ...
      ((= days-of-week 6) "Sa.")
      ((= days-of-week 0) "Su.")
      (else (error "you reach unreachable section" days-of-week)))))

;; データ
(define twenty-century-and-one (iota 101 1900 1))
  ...
  (apply append
    (map (lambda (year)
          (map (lambda (month)
                (list year month 1))
            (iota 12 1))) ;; 1〜12月
      twenty-century-and-one))) ;; 1900〜2000年

;; 残り年数を返す
(define (remaining-years year)
